...
export const runtimeState = {
  ...
  listHosts(): HostRecord[] {
    return state.hosts.map(sanitizeHost).sort((left, right) => left.name.localeCompare(right.name));
  },
  ...
  recordThread(hostId: number, projectId: number | null, thread: any) {
    ...
    const index = state.threadMetadata.findIndex(
  ...
  listThreadMetadata(
    hostId: number,
    options: { projectId?: number | null; cwd?: string | null } = {},
  ) {
