  ...
  async connect() {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const fail = (error: unknown, phase: "websocketHandshake" | "sshChannel") => {
        ...
        if (opened) this.closeFromRemote(transportError.message, closeDetail, transportError);
      };
      ...
      channel.on("close", (code: number | null, signal: string | null) => {
        ...
        if (opened) this.closeFromRemote(error.message, closeDetail, error);
      });
      ws.on("close", () => {
        const detail = {
          code: null,
        ...
        if (opened) this.closeFromRemote(error.message, detail, error);
      });
    });
  }
