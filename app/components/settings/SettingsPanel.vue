  ...
  <Tabs data-testid="settings-panel" default-value="config" class="flex min-h-0 flex-1 flex-col">
      ...
      <div class="grid gap-4 md:grid-cols-2">
        <div class="space-y-3">
          ...
          <Select v-model="hostForm.authMode">
            <SelectTrigger data-testid="host-auth-select" class="w-full bg-white" :aria-label="t('app.auth')">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
          ...
          <Input
            ...
            placeholder="SSH 密码"
          />
          <Button data-testid="add-host-button" class="w-full" :disabled="!hostForm.name || !hostForm.sshHost" @click="createHost">
    ...
    <TabsContent value="projects" class="min-h-0 flex-1 space-y-3">
      ...
      <div class="grid grid-cols-[1fr_auto] gap-2">
        ...
        <Button variant="secondary" :disabled="!selectedHostId || browsing" @click="browseDirectories">
      ...
      <div v-if="directoryError" class="rounded-md bg-red-50 p-3 text-sm text-red-700">{{ directoryError }}</div>
      <Separator />
      ...
      <Button data-testid="add-project-button" class="w-full" :disabled="!selectedHostId || !projectForm.name || !projectForm.remotePath" @click="createProject">
