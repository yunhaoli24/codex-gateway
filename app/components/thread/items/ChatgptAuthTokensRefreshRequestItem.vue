  ...
  <div class="max-w-4xl rounded-lg border border-amber-300/70 bg-amber-50 px-3 py-3 text-sm text-amber-950">
    ...
    <div class="mt-3 grid gap-2 rounded-md bg-white/80 p-3">
      ...
      <div v-if="params.previousAccountId">
        <div class="text-xs font-medium uppercase text-amber-700">{{ t('app.previousAccount') }}</div>
        <div class="mt-1 font-mono text-xs">{{ params.previousAccountId }}</div>
      </div>
      <Input v-model="accessToken" type="password" autocomplete="off" :placeholder="t('app.accessToken')" />
    ...
    <div class="mt-3 flex gap-2">
      <Button size="sm" :disabled="responding || !accessToken || !accountId" data-testid="chatgpt-auth-refresh-submit" @click="submit">
