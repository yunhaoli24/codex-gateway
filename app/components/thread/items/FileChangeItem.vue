...
import { highlightCode, languageFromPath } from '@/utils/code-highlight'
...
const itemStatus = computed(() => typeof props.item.status === 'string' ? props.item.status : props.item.status?.type)
...
<template>
  <div class="max-w-4xl text-[#5f6970]">
    ...
    <div v-if="pendingApproval" class="mt-3 rounded-lg border border-amber-300/70 bg-amber-50 px-3 py-2 text-sm text-amber-900">
      ...
      <div class="mt-2 flex flex-wrap gap-2">
        <Button size="sm" :disabled="responding" data-testid="file-approval-accept" @click="respond('accept')">
          {{ t('app.approve') }}
        </Button>
        <Button size="sm" variant="outline" :disabled="responding" data-testid="file-approval-decline" @click="respond('decline')">
          {{ t('app.decline') }}
        </Button>
      </div>
    </div>
    <div v-if="fileChanges.length" class="mt-3 space-y-2">
      <ScrollArea
        v-if="output"
        class="h-56 rounded-lg border border-black/10 bg-[#f6f6f6]"
      >
        <pre
          class="syntax-highlight p-3 text-xs leading-5 text-[#3d4145]"
      ...
      >
        ...
        <CollapsibleContent>
          ...
          >
            <MarkdownContent :content="diffMarkdown(change)" :diff-language="changeLanguage(change)" compact />
          </StickToBottomScrollArea>
    ...
    <ScrollArea
      v-else-if="output"
      class="mt-3 h-56 rounded-lg border border-black/10 bg-[#f6f6f6]"
    >
      <pre
        class="syntax-highlight p-3 text-xs leading-5 text-[#3d4145]"
        v-html="highlightedOutput"
    ...
    <div v-else-if="isInProgress" class="mt-3 rounded-lg border border-black/10 bg-[#fbfbfb] px-3 py-2 text-sm text-[#8d9499]">
