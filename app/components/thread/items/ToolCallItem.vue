...
const detailSections = computed(() => {
  ...
  if (item.type === 'mcpToolCall') {
    ...
    if (item.error?.message) sections.push({ label: t('app.error'), content: item.error.message, markdown: true })
    ...
    if (item.result) sections.push({ label: t('app.result'), content: item.result, markdown: true })
    if (item.savedPath) sections.push({ label: t('app.savedPath'), content: String(item.savedPath) })
  } else if (item.type === 'enteredReviewMode' || item.type === 'exitedReviewMode') {
    if (item.review) sections.push({ label: t('app.review'), content: item.review, markdown: true })
  }
