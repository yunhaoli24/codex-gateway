...
<template>
  <div class="max-w-[840px] rounded-lg border border-black/10 bg-[#fbfbfb] px-4 py-3 text-[#3d4145]">
    ...
    <div v-if="steps.length" class="mt-3 space-y-2">
      ...
      >
        <CheckCircle2Icon v-if="stepStatus(step) === 'completed'" class="mt-1 size-4 shrink-0 text-emerald-600" />
        <ClockIcon v-else-if="stepStatus(step) === 'inProgress'" class="mt-1 size-4 shrink-0 text-sky-600" />
