...
const elapsedMs = computed(() => Math.max(0, (inProgress.value ? now.value : completedAt.value ?? now.value) - startedAt.value))
...
<template>
  <div class="max-w-4xl overflow-hidden rounded-lg border border-amber-200/80 bg-amber-50/80 text-[#695b3f]">
    <div class="relative">
      <div
      ...
      <div class="relative flex items-center gap-2 px-3 py-2 text-[0.9375rem]">
        ...
        <span class="rounded-full bg-white/80 px-2 py-0.5 font-mono text-xs text-[#5f6970]">{{ timeLabel }}</span>
