...
<template>
  <DropdownMenu>
    <DropdownMenuTrigger as-child>
      <Button type="button" variant="ghost" size="lg" class="min-w-0 flex-1 gap-2 px-2 text-sm font-normal text-[#4f575e] hover:bg-black/[0.04] sm:flex-none md:text-base" data-testid="model-select" :disabled="loadingModels || !models.length">
        <span class="truncate text-[#202225]">{{ loadingModels ? t('app.loadingModels') : activeModelLabel }}</span>
        <span v-if="activeEffortCompactLabel" class="hidden shrink-0 text-[#858b91] sm:inline">{{ activeEffortCompactLabel }}</span>
