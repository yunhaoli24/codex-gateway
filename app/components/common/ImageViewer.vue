...
const imageTransform = computed(() => `translate(${offset.value.x}px, ${offset.value.y}px) scale(${scale.value})`)
...
<template>
  <Dialog v-model:open="open">
    <DialogContent class="flex h-[92vh] w-[96vw] max-w-none grid-rows-none flex-col gap-0 overflow-hidden p-0 sm:max-w-none" :show-close-button="true">
      ...
      >
        ...
        >
        <div class="pointer-events-none absolute left-3 top-3 inline-flex items-center gap-1.5 rounded-md border border-black/10 bg-white/90 px-2 py-1 text-xs text-[#5f6970] shadow-sm">
          <MoveIcon class="size-3.5" />
          {{ zoomLabel }}
        </div>
      </div>

      <div class="flex shrink-0 flex-wrap items-center justify-between gap-3 border-t border-black/10 bg-white px-4 py-3">
        <div class="flex min-w-0 flex-1 items-center gap-2">
          <Button type="button" variant="outline" size="icon-sm" :aria-label="t('app.zoomOut')" @click="zoomBy(-0.25)">
            <ZoomOutIcon class="size-4" />
          </Button>
          <Slider v-model="zoomValue" class="max-w-48" :min="25" :max="600" :step="5" :aria-label="t('app.imageZoom')" />
          <Button type="button" variant="outline" size="icon-sm" :aria-label="t('app.zoomIn')" @click="zoomBy(0.25)">
            <ZoomInIcon class="size-4" />
          </Button>
          <Button type="button" variant="ghost" size="sm" class="min-w-16" :aria-label="t('app.resetImageView')" @click="resetView">
