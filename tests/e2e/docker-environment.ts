...
export async function startDockerEnvironment() {
  ...
  env.initialCodexVersion = parseCodexVersionOutput((await execRemoteSsh(env, `${remoteCodexBin} --version`)).stdout)
...
async function waitForSsh(host: string, port: string) {
  ...
  while (Date.now() < deadline) {
    try {
      ...
      const connection = await connectSsh({ host, port, username: 'codex', password: process.env.E2E_REMOTE_PASSWORD || 'codex' })
      connection.end()
      return
    } catch (error: any) {
...
async function writeRemoteImage(env: RemoteEnv) {
  const connection = await connectSsh(env)
  try {
    await execSsh(connection, `printf %s iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR4nGP4z8DwHwAFAAH/iZk9HQAAAABJRU5ErkJggg== | base64 -d > ${env.imagePath}`)
  } finally {
    connection.end()
  }
}
...
async function uploadDirectory(connection: Client, localDirectory: string, remoteDirectory: string) {
...
async function uploadDirectoryEntries(sftp: import('ssh2').SFTPWrapper, localDirectory: string, remoteDirectory: string) {
