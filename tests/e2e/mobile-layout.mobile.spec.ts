...
test('opens sidebar context actions with long press on mobile', async ({ page }) => {
  ...
  if (!await page.getByTestId('settings-toggle').isVisible().catch(() => false)) {
    await page.getByTestId('mobile-sidebar-toggle').click()
  }
...
async function createConfiguredHostAndProject(page: Page, remote: Awaited<ReturnType<typeof readRemoteEnv>>) {
  const host = await page.evaluate(async ({ remote }) => {
  ...
  const project = await page.evaluate(async ({ hostId, remote }) => {
    ...
    return await response.json()
  }, { hostId: host.id, remote }) as { id: number }
  await page.evaluate((hostRecord) => {
    localStorage.setItem('codex-gateway-config', JSON.stringify({
      version: 1,
      hosts: [hostRecord],
      pinnedThreads: [],
      lastOpenThread: null,
